...
(function($, window, document) {
  ...
  */
  var MAX_LENGTH_ALLOWED_KEYS = [8, 9, 17, 18, 37, 38, 39, 40, 91, 46,13];
  ...
  var EmojiArea_WYSIWYG = function($textarea, id, options) {
    ...
    this.$editor.on("change keydown keyup resize scroll", function(e) {
      if(MAX_LENGTH_ALLOWED_KEYS.indexOf(e.which) == -1 &&  !((e.ctrlKey || e.metaKey) && e.which == 65) && !((e.ctrlKey || e.metaKey) && e.which == 67) &&  editorDiv.text().length + editorDiv.find('img').length >= editorDiv.attr('maxlength'))
      {
        e.preventDefault();
      }
